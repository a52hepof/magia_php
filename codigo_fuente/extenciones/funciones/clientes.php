...
function clientes_add($selecionado = "", $excluir = "") {
    ...
    while ($clientes = mysql_fetch_array($sql)) {
        ...
        echo "value=\"$clientes[email]\">" . strtoupper($clientes['empresa']) . " - $clientes[cliente] ($clientes[email])</option>";
    }
}
...
function clientes_tabla_index_titulo($orden) {
    ...
    echo "      
                <th><a href=\"index.php?p=clientes&c=index&ordenpor=empresa&orden=$ad\">" . _tr("Empresa") . "</a> $icono</th> 
                <th><a href=\"index.php?p=clientes&c=index&ordenpor=cliente&orden=$ad\">" . _tr("Cliente") . "</a> $icono</th>";
